package controls
{
	...
	public class ThirdControl implements Control
	{
		public function angleCheck(player:FlxSprite):Number{
			var angle:Number = 0;
			if(FlxG.keys.justPressed("I") && player.aimAngle > -Math.PI/2 + .01){
				player.aimAngle -= Math.PI/4;
			}
			if(FlxG.keys.justPressed("K") && player.aimAngle < Math.PI/2 - .01){
				player.aimAngle += Math.PI/4;
			}
			if(player.isLeft){
